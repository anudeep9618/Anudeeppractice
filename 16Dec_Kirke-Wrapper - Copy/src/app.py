...
async def withdraw_request(request_id: int = Path(..., description="Request ID to withdraw"),
...
@app.get("/kirke-wrapper/getdevice/{device_name}", response_model=GetDeviceResponsePayload,
          responses = {
              500: {"model": FailureResponse, "description": "Internal Server Error"},
              403: {"model": FailureResponse, "description": "Forbidden Error"},
          }
          )
async def get_device(device_name: str = Path(..., description="device_name to get kirke"),
                      authorization: str = Header(..., description="token"),
                      start_date_time: str = Query(..., description="Start date"),
                      end_date_time: str = Query(..., description="End date"),
                      start_date_offset: str = Query(..., description="Plus or Minus 3"),
                      end_date_offset: str = Query(..., description="Plus or Minus 3")
    ):
    vzLog.log_info(
        f"Received get_device for the device name {device_name}"
    )
    try:
        # validate token
        validated = await request_handlers.handle_validate_token(authorization)
        if validated:
            # request handler
            await request_handlers.handle_get_device(device_name, start_date_time, end_date_time, start_date_offset, end_date_offset)
        else:
            raise HTTPException(
                status_code=403,
                detail="Forbidden: Access is denied" 
            )
        #return GetDeviceResponsePayload(device_name="")
        
    except HTTPException as e:
        vzLog.log_error(e.detail)
        return JSONResponse(
            content=str(e), #FailureResponse(message = e.detail, status_code = e.status_code).model_dump(),
            media_type="application/json",
            status_code=e.status_code,
        )
    except Exception as e:
        ...
        return JSONResponse(
            content= str(e),
            # FailureResponse("get_device method failed").model_dump(),
            media_type="application/json",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
